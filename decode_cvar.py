...
@torch.no_grad()
def controlled_decode_smc_local(model, gen_batch_num: int, n_tasks: int, alpha: float, cvar_beta: float, cvar_eta: float, cvar_lambda: float = 1.0, variant: str = "MC", x0_mode: str = "soft", reward_channel: int = 0, show_progress: bool = True):
    variant = variant.upper()
    if variant not in {"MC", "PM"}:
        raise ValueError(f"variant must be 'MC' or 'PM', got: {variant!r}")
